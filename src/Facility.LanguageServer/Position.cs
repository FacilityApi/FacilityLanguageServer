...
namespace Facility.LanguageServer
{
	struct Position : IComparable<Position>, IEquatable<Position>
	{
		...
		//public static implicit operator Position?(NamedTextPosition position) => position != null ? new Position(position) : default(Position?);

		//public static implicit operator Position?(LspPosition position) => position != null ? new Position(position) : default(Position?);

		public static implicit operator LspPosition(Position position) => new LspPosition(position.Line, position.Character);

		//public static explicit operator LspRange(Position position) => new LspRange(position, position);
	...
	{
		//public static Position? AsPosition(this NamedTextPosition position) => position != null ? new Position(position) : default(Position?);
	}
}
